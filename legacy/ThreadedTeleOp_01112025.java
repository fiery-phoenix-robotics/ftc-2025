...
@com.qualcomm.robotcore.eventloop.opmode.TeleOp(name="ThreadedTeleOp_01112025", group="Competition")
public class ThreadedTeleOp_01112025 extends LinearOpMode {
    ...
    public static Long lastLeverSpeedCheck = null;
    public static int leverPositionAtCheck;

    public static Long lastArmSpeedCheck = null;
    public static int armPositionAtCheck;
    
    public static Long lastWristSpeedCheck = null;
    public static int wristPositionAtCheck;
    ...
    public enum TowerAction implements Runnable {
        ...
        MANUAL_MOVE_WRIST {
            public void run() {
                ...
                } else if (calculateWristSpeed() <= 0) {
                    targetWrist = wrist.getCurrentPosition();
    ...
    public void runOpMode () {
        ...
        while (opModeIsActive()) {
            ...
            telemetry.addData("OTOS Reading", "(" + pos.x + ", " + pos.y + ") at " + pos.h + " degrees");
    ...
    public static void moveWristTo (int pos, double power) {
        ...
        while (calculateWristSpeed() > 0 && !(pos - wristMarginOfError < wrist.getCurrentPosition() && wrist.getCurrentPosition() < pos + wristMarginOfError)) {
    ...
    public static void moveLeverTo (int pos, double power) {
        ...
        while (calculateLeverSpeed() > 0 && !(pos - leverMarginOfError < lever.getCurrentPosition() && lever.getCurrentPosition() < pos + leverMarginOfError)) {
    ...
    public static double calculateLeverSpeed () {
        double leverSpeed = -1.0;
        Long currentTime = Long.valueOf(System.currentTimeMillis());
        int currentLeverPosition = lever.getCurrentPosition();
        if (currentTime % 50 == 0 && currentTime != lastLeverSpeedCheck) {
            if (lastLeverSpeedCheck != null) {
                leverSpeed = (double)Math.abs((currentLeverPosition - leverPositionAtCheck) / (currentTime - lastLeverSpeedCheck));
            }
            else {
                leverSpeed = Math.abs(currentLeverPosition / 20);
            }
        }
        lastLeverSpeedCheck = currentTime;
        return leverSpeed;
    }
    
    public static double calculateArmSpeed () {
        double armSpeed = -1.0;
        Long currentTime = Long.valueOf(System.currentTimeMillis());
        int currentArmPosition = arm.getCurrentPosition();
        if (currentTime % 50 == 0 && currentTime != lastArmSpeedCheck) {
            if (lastArmSpeedCheck != null) {
                armSpeed = (double)Math.abs((currentArmPosition - armPositionAtCheck) / (currentTime - lastArmSpeedCheck));
            }
            else {
                armSpeed = Math.abs(currentArmPosition / 20);
            }
        }
        lastArmSpeedCheck = currentTime;
        return armSpeed;
    }
    
    public static double calculateWristSpeed () {
        double wristSpeed = -1.0;
        Long currentTime = Long.valueOf(System.currentTimeMillis());
        int currentWristPosition = wrist.getCurrentPosition();
        if (currentTime % 50 == 0 && currentTime != lastWristSpeedCheck) {
            if (lastWristSpeedCheck != null) {
                wristSpeed = (double)Math.abs((currentWristPosition - wristPositionAtCheck) / (currentTime - lastWristSpeedCheck));
            }
            else {
                wristSpeed = Math.abs(currentWristPosition / 20);
            }
        }
        lastWristSpeedCheck = currentTime;
        return wristSpeed;
    }
