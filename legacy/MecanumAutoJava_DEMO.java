...
@Autonomous(name = "MecanumAutoJava_DEMO", group="Competition", preselectTeleOp="StarterBot2025_FINAL_2DRIVER")
public class MecanumAutoJava_DEMO extends LinearOpMode {
  ...
  private ElapsedTime runtime = new ElapsedTime();
    ...
    if (opModeIsActive()) {
     // Create target positions, use trig to calculate factor to multiply by
      frontLeftTarget = leftDriveFront.getCurrentPosition() + (int)(r * (Math.sin(thetaAsRadians) - Math.cos(thetaAsRadians)) * DRIVE_COUNTS_PER_IN);
      frontRightTarget = rightDriveFront.getCurrentPosition() - (int)(r * (Math.sin(thetaAsRadians) + Math.cos(thetaAsRadians)) * DRIVE_COUNTS_PER_IN);
    ...
    private void demoCycle(double power) {
        drive(power, 48, 0);
        drive(power, 48, 180);
    }
